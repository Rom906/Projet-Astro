...
T_x = (
    444.3807 + 0.02385 * T_inf - 392.8292 * e ** (-0.0021357 * T_inf)
)  # K, température at z_x
...
def T(z: float) -> float:
    """
    Gives temperature at z altitude used in Jacchia 77's model
...
def concentration_ni_z0(molecular_index: int) -> float:
    """
    Gives density in number of an atmospheric molecule at reference altitude of 120 km using data from NRLMSIS 2.0 nominal 05/04/2026 55°/45°

    :param molecular_index: index of the molecule in molecules_list
    :type molecular_index: int
    :rtype: float
    """
    ni_z0 = [
        4.43 * (10**16) / Na,
...
def concentration_ni(z: float, molecular_index: int) -> float:
    """
    Gives density in number of an atmospheric molecule at z altitude used in Jacchia 77's model
    ...
    """
    return (
        concentration_ni_z0(molecular_index)
        * (
            ((1 - a) / (1 - a * e ** (-D * x(z))))
            ** (1 + alpha_i(molecular_index) + gamma_i(molecular_index))
        )
        * e ** (-D * gamma_i(molecular_index) * x(z))
    )  # à changer aavec ni_zx
