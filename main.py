...
from normalization import NormalizationParameters, differential_equation_normalized, convert_to_dimensional
...
solution_normalized, time_noramlized = compute_solution(RK4, differential_equation_normalized, 2000000, 0, 1000, Vector([Vector([0, 1, 0]), Vector([0, -0.05, -0.05])]))

position_normalized = solution_normalized[0]
velocity_normalized = solution_normalized[1]
position, velocity = convert_to_dimensional(position_normalized, velocity_normalized, parameters)
